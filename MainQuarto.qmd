...
write.csv(high_conf, "..data/processed/All_TSL_transcripts.csv", row.names = FALSE)
...
writeLines(all_transcript_ids, "..data/temp/all_transcript_ids.csv") 
...
file <- "../results/TargetP/mitocarta_output.txt"
...
# Save
write.csv(mtp_pos, "..results/tables/MTS_Predictions.csv", row.names = FALSE)
...
write.csv(sp_info, "..results/tables/SP_transcripts.csv", row.names = FALSE)
...
ggsave("/results/figures/gene_wise_mts_classification.png",
...
# Load the data
gene_summary_mts = pd.read_csv("..data/temp/gene_summary_mts.csv")
...
# Save the figure
plt.savefig("../results/figures/transcript_composition_heatmap.png", dpi=300, bbox_inches='tight')
...
writeLines(mts_transcript_ids, "../data/temp/mts_transcript_ids.csv")  
```
...
output_file <- "../data/temp/processed_tpm.txt"
...
input_file="../data/temp/processed_tpm.txt"
output_file="/data/processed/GTEX_tpm_avg.tsv"
...
sample_attributes <- read.delim("/data/metadata/GTEx_Analysis_v10_Annotations_SampleAttributesDS.txt", 
                                header = TRUE, 
                                sep = "\t", 
...
# Read the TSV file
processed_tpm_avg <- read_tsv("../data/processed/GTEX_tpm_avg.tsv")
...
To identify tissue-specific dominant transcripts, 2,799 transcripts from the mtp_pos matrix were filtered. TPM values were converted to within-gene expression percentages per tissue, and transcripts contributing ≥60% of their gene's total expression in a given tissue were classified as dominant.
...
write.csv(all_switching_genes, "../results/tables/mito_switching_genes.csv", row.names = FALSE)
...
# To find main dominant transcript per gene
...
write.csv(dominant_transcripts, "/data/temp/dominant_transcripts.csv", row.names = FALSE)
write.csv(filtered_percentages, "/data/temp/filtered_percentages.csv", row.names = FALSE)
write.csv(all_switching_genes, "/data/temp/all_switching_genes.csv", row.names = FALSE)
writeLines(tissue_cols, "/data/temp/tissue_cols.txt")
...
# Load data
all_switching_genes = pd.read_csv("../data/temp/all_switching_genes.csv")
filtered_percentages = pd.read_csv("../data/temp/filtered_percentages.csv")
...
# Map transcripts to colours for heatmap
...
# For stacked bar data
# Filter filtered_percentages for a gene of choice
genes_to_plot = ['ENSG00000104936']  
...
# Custom colours for transcripts
...
sns.despine(ax=ax2)
plt.tight_layout()
plt.savefig("../results/figures/stacked_bar_heatmap_plot.png", dpi=300)
plt.show()
...
write_csv(switching_long, "../data/temp/all_switching_gene_transcripts.csv")
...
TRANSCRIPTS_CSV="../data/temp/all_switching_gene_transcripts.csv"
GTF="../data/gencode.v47.annotation.gtf"
COMBINED_OUTPUT="../data/processed/all_genes_unique_exons.csv"
...
allbed_file <- read.csv("../data/processed/all_genes_unique_exons.csv")
...
write.csv(all_clin_filtered, file = "../results/tables/mitocarta_disease.csv", row.names = FALSE)
...
DRIMSeq analyse differential transcript usage between conditions or groups using RNA-Seq data. It focuses on isoform-level expression and can help assess whether different versions of a gene (transcripts/isoforms) are differentially expressed across different conditions or tissue types.
...
-   **pvalue =** The raw p-value for differential transcript usage between liver and muscle. If pvalue is very **small**, this suggests strong evidence that this gene’s transcript usage differs significantly between liver and muscle.
...
Merge Datasets with MTS status for future evaluation
...
The short-read TPM expression analysis was repeated with a focus on MTS-based switching—identifying changes in dominant transcript usage between MTS-positive and MTS-negative isoforms across tissues.
...
Genes were then identified where only one MTS category—either MTS-positive or MTS-negative—was exclusively expressed, accounting for 100% of the gene’s expression within a given tissue.
...
For each gene, the MTS status (positive or negative) that accounted for more than 75% of total expression in a given tissue was identified.
...
# Save 
output_path <- "../results/figures/all_APA_Clin_Table_named.docx"
...
# Save 
writeLines(clinvar_filtered$vep_input, "../data/processed/vep_input.txt")
...
prompts_table <- read.table("../results/vep_results.txt", header = TRUE, stringsAsFactors = FALSE)
...
fasta_file <- readAAStringSet("../data/clinvar_fasta.txt")
...
# Save
writeXStringSet(fasta_file, filepath = "../data/temp/clinvar_modified.fasta")
...
mtp_table <- read.table(
  "../results/TargetP/vep_targetp.txt",
  header = TRUE,
  sep = "\t",
  stringsAsFactors = FALSE
)
...
# Load counts file
counts <- read.delim("../nanopore/results/bambu/counts_transcript.txt", header = TRUE, stringsAsFactors = FALSE)
gtex_annot <- read_excel("../nanopore/metadata/41586_2022_5035_MOESM4_ESM.xlsx")
```
...
# Load GTF
gtf <- import("../nanopore/results/bambu/extended_annotations.gtf")
...
# Load transcript list that has all the switching list from short read analysis

combined_transcripts <- read.csv("..data/temp/combined_transcripts.csv", header = FALSE)
...
short_common <- short_tpm_filtered %>%
  select(transcript_id, gene_id, all_of(common_tissues))

long_common <- long_tpm_filtered %>%
  select(TXNAME, GENEID, all_of(common_tissues))
...
# Select dominant transcript per gene per tissue
...
# Filter all_switching_genes_long 
...
colnames(filtered_df)[1] <- "gene_id" 
...
Check for common switching genes between both short read and long read analysis.
...
# Filter to keep only matching genes
...
Move all the .bed files into a new directory bed_files. 
...
bed_path <- "../nanopore/results/minimap2/chrM_extraction_results/bed_files/"
...
gtf <- import("../data/gencode.v47.annotation.gtf")
...
# Set paths
bed_dir <- "../nanopore/results/minimap2/chrM_extraction_results/bed_files/"
gtf_path <- "../data/gencode.v47.annotation.gtf"
...
# Save
output_path <- "../results/figures/genic_novel_site_summary.docx"
